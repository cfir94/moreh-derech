    ...
    <section aria-label="מחברות השיעורים המקוונים">
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-txt-dim">המחברות מוצגות לפי סדר ההוספה.</p>
        </div>
...
export function NotebookLibrary() {
  ...
  const tourNotebooks = useMemo<CourseNotebook[]>(() => [
    ...TOUR_NOTEBOOKS,
    ...storedNotebooks
      .filter((notebook) => notebook.mode === "tours" && TOUR_REGIONS.some((region) => region.id === notebook.subcategory))
      .map((notebook) => ({
  ...
  const theoryNotebooks = useMemo<TheoryNotebook[]>(() => [
    ...THEORY_NOTEBOOKS,
    ...storedNotebooks
      .filter((notebook) => notebook.mode === "theory" && THEORY_SUBJECTS.some((subject) => subject.id === notebook.subcategory))
      .map((notebook) => ({
        id: `stored-${notebook.id}`,
  ...
  const shownNotebooks = useMemo(
    () => activeRegion === "all" ? tourNotebooks : tourNotebooks.filter((notebook) => notebook.region === activeRegion),
    [activeRegion, tourNotebooks],
  );
  ...
  return (
    <main className="screen-in mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10" dir="rtl">
      ...
      {activeMode === "online" ? (
        <OnlineSection notebooks={onlineNotebooks} />
      ) : activeMode === "theory" ? (
        <TheorySection notebooks={theoryNotebooks} />
      ) : (
        <>
          ...
          <section aria-live="polite" aria-label="מחברות הסיורים">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-xl text-txt sm:text-2xl">{activeRegion === "all" ? "כל מחברות הסיורים" : TOUR_REGIONS.find((region) => region.id === activeRegion)?.title}</h2>
                {activeRegion !== "all" && <p className="mt-1 text-sm text-txt-dim">{TOUR_REGIONS.find((region) => region.id === activeRegion)?.description}</p>}
              </div>
              <p className="text-xs font-bold text-txt-dim">{shownNotebooks.length} קישורים ישירים למחברות</p>
            </div>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
