...
- **מבחן מלא** — מבחן רישוי שלם בתנאי אמת, בשלוש שפות (`/exams`).
...
- `src/data/quizzes/past-exams.ts` — 185 שאלות ממבחני הרישוי הרשמיים של משרד
...
- **שלוש שפות** (עברית/אנגלית/ערבית) — המבחן מתפרסם בשלוש מהדורות, וכל אחת
  נמצאת ב-`src/data/exams/july-2024.ts`. ה-chrome של האתר נשאר עברית; רק תוכן
  המבחן וכיוון הכתיבה מתחלפים.

**האינטגרציה עם המעקב**: תשובות במבחן נרשמות תחת השאלון `past-exams` ולפי
מזהה השאלה המקבילה בו (`quizId`), ולכן טעות במבחן נכנסת ל"תרגול טעויות"
הרגיל. לכן `src/data/exams/july-2024.ts` ובלוק "יולי 2024" שבתוך
`src/data/quizzes/past-exams.ts` **נוצרים יחד** ע"י `tools/exam_to_site.py` —
אין לערוך אחד מהם בנפרד.
...
#### חילוץ מבחן שלם (`tools/parse_exam.py` + `build_exam.py` + `exam_to_site.py`)

הסקריפט הישן איבד שאלות בשקט (יולי 2024 יצא <span dir="ltr">29</span> שאלות
במקום <span dir="ltr">33</span>). המחליף עובד **לפי פריטים ממוספרים** על זרם
שורות אחד, ולכן תופס גם אופציה שהתווית והטקסט שלה נפלו לשורות נפרדות, אופציה
שגלשה, ופריט שנחתך בין עמודים. נקודות שחשוב להכיר לפני הרצה על מבחן חדש:
...
- **טקסט נבנה מהגליפים של כל מילה**, לא מהיפוך המחרוזת: היפוך מחרוזת שובר את
  ...
  תווים. סימני ניקוד ערביים (tashkeel) נמחקים — הם מגיעים כתיבות ברוחב אפס
  ומפצלים מילה לשתיים.
- **שיוך גליף לשורה לפי חפיפה אנכית של יותר מחצי גובהו** — שורות סמוכות
  נושקות זו לזו, וסף רופף יותר משרשר שתי שורות זו לתוך זו.
- **אימות בין שפות**: אותו מבחן משוחרר בשלוש מהדורות המסומנות בנפרד, ולכן
  התשובה הנכונה חייבת לשבת באותו אינדקס בכולן. ב-2024: אנגלית
  <span dir="ltr">32/32</span>, ערבית <span dir="ltr">33/33</span> תואמות
  לעברית. אי-הסכמה = שגיאת פרסינג, והשאלה נזרקת. (באנגלית שאלה
  <span dir="ltr">20</span> מסומנת בארבע תשובות ב-PDF המקורי — האינדקס נלקח
  מעברית.)
- **שני חלקי הפריט נשמרים** — גם משפט ההשלמה וגם התשובה שהמפתח מדפיס לו
  בסוגריים — ולא רק שאלת ההבנה, כי במצב מבחן שניהם מוצגים.

הרצה: `python3 tools/build_exam.py > /tmp/exam.json` (מתוך תיקיית ה-PDF-ים,
עם עדכון `SOURCES`) ואז `python3 tools/exam_to_site.py /tmp/exam.json`.
...
src/
  app/
    ...
    exams/                    אינדקס מבחנים מלאים
      [slug]/                 מבחן בתנאי אמת (3 שפות, הגשה בסוף)
